<template>
    <div>
        <div class="horror-movie relative grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8">
...
export default {
    ...
    mounted () {
        this.movieSeries = this.movieData.filter(product=> product.category.includes("horror"))
    },
    computed: {
        getItems() {
...
.pagination-list{
    ...
    li {
        ...
        a {
            @apply w-[35px] h-[35px] text-[16px] flex justify-center items-center rounded-full mx-1 text-white text-center bg-[#333333] hover:bg-green-brand;
        }
    }
}
</style>